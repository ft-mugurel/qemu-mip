...
/*
 * Fallback only when socket_thread=off: drive the server from TB translate.
 * With the dedicated thread (default), this callback is not registered —
 * otherwise agents would only get replies while the guest is translating.
 */
